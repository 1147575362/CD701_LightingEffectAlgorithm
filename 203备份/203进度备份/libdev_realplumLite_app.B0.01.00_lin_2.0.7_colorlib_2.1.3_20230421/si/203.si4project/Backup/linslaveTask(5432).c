...
#define FID_LEFT_CTRL_INDEX     	(6)
#define FID_RIGHT_CTRL_INDEX     	(7)

#define PROJECT_NAME				201

#define COLOR_TABLE_SIZE  64						//copy
static  RGB_xy_t  rgbTable[COLOR_TABLE_SIZE] = {	
    {0x0054FF, 0.1621, 0.1036},   //1
    {0x1153FF, 0.1632, 0.1033},   //2
...
static LIN_Device_Frame_t UnconditionalCmdsTable[DEFAULT_LINS_FID_SIZE] = {
  ...
  [FID_RIGHT_CTRL_INDEX] = {				// 右门控制
		.frame_id = RIGHT_CONFIG,	 		// 0x04
		.msg_type = LIN_MSG_TYPE_RX,		 
		.checksum = LIN_CHECKSUM_ENHANCED,		 
		.length   = LIN_BUFF_SIZE,
		.frameIsValid = TRUE,
...
void UnconditionalSubscribedCmdsHandle(LIN_Device_Frame_t const *frame)
{
	...
	if (frame->frame_id == UnconditionalCmdsTable[FID_LEFT_CTRL_INDEX].frame_id)	//进入左门FID处理函数
	{
		// 处理颜色
		uint8_t Colour_Buff = frame->data[4];
		uint8_t Colour_Red = rgbTable[Colour_Buff].RGB >> 16;
		uint8_t Colour_Green = (rgbTable[Colour_Buff].RGB >> 8) & 0xff;
		uint8_t Colour_Blue = (rgbTable[Colour_Buff].RGB) & 0xff;
		// 处理亮度
		uint8_t DimLevel_Buff = frame->data[5]; 
        (void)CLM_SetRGBL(LED0,MES_GetLedTemperature(LED0), Colour_Red, Colour_Green, Colour_Blue,(uint16_t)DimLevel_Buff*16U, (uint16_t)100U);
    }

	if (frame->frame_id == UnconditionalCmdsTable[FID_RIGHT_CTRL_INDEX].frame_id)	//进入左门FID处理函数
	{
        // 处理颜色
		uint8_t Colour_Buff = frame->data[4];
		uint8_t Colour_Red = rgbTable[Colour_Buff].RGB >> 16;
...
void APPL_HandleLeftDoor(LIN_Device_Frame_t const *frame)
{}

void APPL_HandleRightDoor(LIN_Device_Frame_t const *frame)
{}
